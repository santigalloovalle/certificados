    ...
    public function passes($attribute, $value)
    {
        ...
        $validator = Validator::make(
            ...
            if ($this->letters && ! preg_match('/\pL/u', $value)) {
                $validator->errors()->add($attribute, 'La contraseña debe contener al menos una letra.');
            }

            if ($this->symbols && ! preg_match('/\p{Z}|\p{S}|\p{P}/u', $value)) {
                $validator->errors()->add($attribute, 'Un Carácter especial requerido.');
            }

            if ($this->numbers && ! preg_match('/\pN/u', $value)) {
                $validator->errors()->add($attribute, 'Un número requerido');
            }
        });
